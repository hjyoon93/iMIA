...
if __name__ == '__main__':
    ...
    if args.dataset == 'mnist':
        ...
        if args.iid:
            ...
            with open('file_arg_20_four.txt', 'a') as file:
                file.write("%i\n" % args.epochs)
                file.close()
            ...
            #print('length0:', len(dict_users['0']))
            #print('length0:', len(dict_users['0']))
            #print('length1:', len(dict_users['1']))
            #print('length2:', len(dict_users['2']))
            #print('length3:', len(dict_users['3']))
            ...
            rsu1_ev = min(np.random.normal(0.6, 0.05), 1)
            while rsu1_ev < 0:
                rsu1_ev = min(np.random.normal(0.6,0.05), 1)

            rsu2_ev = min(np.random.normal(0.6, 0.05), 1)
            while rsu2_ev < 0:
                rsu2_ev = min(np.random.normal(0.6, 0.05), 1)
            ...
            rsu1_sv = min(np.random.normal(0.7, 0.05), 1)
            while rsu1_sv < 0:
                rsu1_sv = min(np.random.normal(0.7, 0.05), 1)
            ...
            rsu3_sv = min(np.random.normal(0.5, 0.05), 1)
            while rsu3_sv < 0:
                rsu3_sv = min(np.random.normal(0.5, 0.05), 1)

            ccs_sv = min(np.random.normal(0.3, 0.05), 1)
            while ccs_sv < 0:
                ccs_sv = min(np.random.normal(0.3, 0.05), 1)

            rsu0_uv = min(np.random.normal(0.6, 0.05), 1)
            while rsu0_uv < 0:
                rsu0_uv = min(np.random.normal(0.6, 0.05), 1)
            ...
            rsu2_mem = min(np.random.normal(0.07, 0.05), 1)
            while rsu2_mem < 0:
                rsu2_mem = min(np.random.normal(0.07, 0.05), 1)

            rsu3_cpu = min(np.random.normal(0.07, 0.05), 1)
            while rsu3_cpu < 0:
                rsu3_cpu = min(np.random.normal(0.07, 0.05), 1)

            rsu3_mem = min(np.random.normal(0.06, 0.05), 1)
            while rsu3_mem < 0:
                rsu3_mem = min(np.random.normal(0.06, 0.05), 1)
            ...
            #new_adding V (v0-v55) #does the vehicle appear in the mission service area?
            for i in range(56):
                List.append('true')
            ...
            for i in car_ac:
                if random.random() < i:
                    List.append('true')
                    car_collect_sv.append('true')
                    #number_service_success += 1
                else:
                    List.append('false')
                    car_collect_sv.append('false')

            print("length of list after s1:", len(List))
            ...
            with open('input_20_four.txt', 'w') as f:
            ...
            with open('input_20_four.txt') as f:
                a = f.read().splitlines()
                #a = a[:56]
    ...
    for iter in range(args.epochs):  #args.epochs is rounds of training. For every round of training
        ...
        print("length of sa_list:", len(sa_list))
        labels = []
        test_train_cases = []
        test_train_cases_new = []
        rsu_store = []
    ...
    #################################start of the rsu and ccs communication##########################################
        ...
        if iter == 0:
            ...
            List.append('true')
            List.append('true')
            total_number_service += 1   #this is commented out because this is always true
            number_service_success += 1
            sa_list.append('true')
            print("leng of list after a2, s2, sv2 in iter1:", len(List))
            acc_test = 0
            print("this is the acc_test", acc_test)
        ####newline
        ...
        if iter >= 1:
            ...
            if node_compromised_NIDS[4] == True: #if ccs is detected as compromised but it was always normal, then service-tasks are false and move to next iteration
                ...
                #newest_add_for 5 services
                for i in range(5):
                    sa_list.append('false')
                #end_newst_add_for 5 services

                if int(acc_test) >= 85: # sv5
                    List.append('true')
                else:
                    List.append('false')
        ...
        if random.random() < 0.4: #60% chance of an attack happening
            ...
            if attack == 6: #random.random() < 0.5: #choose to perform either dos or phishing+dp
                if random.random() < vul[4]: #(SV[victim] * UV[victim]): #compromised; #########then node being attacked is compromised by dos
                    ...
                    ccs_asset[0][0] += (ccs_asset[0][0] * 0.5)
                    ccs_asset[0][1] += (ccs_asset[0][1] * 0.5)
            ...
            if attack == 1: #random.random() < 0.5: #choose to perform either dos or phishing+dp
                if random.random() < vul[victim]: #(SV[victim] * UV[victim]): #compromised; #########then node being attacked is compromised by dos
                    ...
                    # asset capacity update
                    rsu_asset[victim][0] += (rsu_asset[victim][0] * 0.5)
                    rsu_asset[victim][1] += (rsu_asset[victim][1] * 0.5)
        ...
        for i in range(len(rsu_participate)):
            if rsu_participate[i] == 'false':
                ...
                #newest_add_for services
                sa_list.append('false')
                #end_newest_add_for services
    ...
    ########newest_adding sv_6 every iter
    count_sv6 = 1
    for l in range(1,101,5):
        for j in range(l,l+5):
            if sa_list[j] == 'true':
                count_sv6 += 1
        if (count_sv6 / (l+5)) >= 0.87:
            List.append('true')
            print('sv6:',(count_sv6 / (l+5)))
        else:
            List.append('false')
            print('sv6:', (count_sv6 / (l + 5)))
    ########end_newest adding_sv_6 every iter
    ...
    ######t/f file
    #file = open("file_arc_d1.txt", "a") #it was file1.txt
    # Saving the array in a text file
    #content = str(List) + '\n'
    #file.write(content)
    #file.close()
    ...
    with open('file_predictability_20_four.txt', 'a') as file:
        file.write("%i\n" % acc_test)
        file.close()

    # ASR file: MPE
    with open('file_asr_20_four.txt', 'a') as file:
        file.write("%f\n" % asr)
        file.close()

    # Timely service availability file: MPE
    with open('file_sa_20_four.txt', 'a') as file:
        file.write("%f\n" % sa)
        file.close()
